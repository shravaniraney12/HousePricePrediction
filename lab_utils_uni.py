...
def plt_contour_wgrad(x, y, hist, ax, w_range=[-1000, 5000, 500], b_range=[-10000, 10000, 50],
...
def plt_gradients(x_train,y_train, f_compute_cost, f_compute_gradient):
    ...
    # Print w vs cost to see minimum
    fix_b = 700
    w_array = np.linspace(0, 10000, 500)
    w_array = np.linspace(500, 1000, 500)
    cost = np.zeros_like(w_array)
    ...
    ax[0].set_title("Cost vs w, with gradient; b set to 700")
    ax[0].set_ylabel('Cost')
    ax[0].set_xlabel('w')

    # plot lines for fixed b=100
    for tmp_w in [500,741,900]:
        fix_b = 700
        dj_dw,dj_db = f_compute_gradient(x_train, y_train, tmp_w, fix_b )
        j = f_compute_cost(x_train, y_train, tmp_w, fix_b)
        add_line(dj_dw, tmp_w, j, 300, ax[0])

    #===============
    # Second Subplot
    #===============

    tmp_b,tmp_w = np.meshgrid(np.linspace(-200, 200, 10), np.linspace(500, 1000, 10))
